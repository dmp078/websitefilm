...
function Bookmarked () {
    document.title = 'Bookmarked | Moonlight'
    ...
    useEffect(() => {
        if (user.email) {
            getDoc (doc (db, 'users', user.email))
                .then (res => {
                    res.data().bookmarked.movie.forEach (e => {
                        getDetail ('movie', +e)
                            .then (movie => setListMovie (pre => [...pre, movie]))
                    })

                    res.data().bookmarked.tv.forEach (e => {
                        getDetail ('tv', +e)
