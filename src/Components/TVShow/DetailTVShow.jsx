...
function DetailTVShow () {
    ...
    const listTab = ['Overall', 'Cast', 'Seasons']
    ...
    useEffect (() => {
        ...
        getDetail ('tv', id)
            .then (res => {
                setObj (res)
                document.title = res.name + ' | Detail'
            })
    ...
    useEffect (() => {
        if (user.email) {
            getDoc (doc (db, 'users', user.email))
                .then (res => {
                    if (res.data().bookmarked.tv.includes (id)) {
                        setHeart (true)
                    } else {
                        setHeart (false)
                    }
        ...
        if (!user.email) {
            ...
            getDoc (doc (db, 'users', user.email))
                .then (res => {
                    ...
                    if (res.data().bookmarked.tv.includes (id)) {
                        setHeart (false)
                        objUpdate.bookmarked.tv = res.data().bookmarked.tv.filter (e => e != id)
                    } else {
                        setHeart (true)
                        objUpdate.bookmarked.tv = [...res.data().bookmarked.tv, id]
                    }
                            ...
                            <div className="relative w-full z-1">
                                ...
                                <div className="flex gap-6 absolute -bottom-20 left-6 z-1">
                                    ...
                                    <div className="relative w-fit ">
                                        ...
                                        <Link to={`/tv/play/${id}`} className="flex gap-6 h-fit mt-4 my-auto bg-blue-500 py-4 px-6 text-white rounded-full">
                            ...
                            <div className="px-6">
                                ...
                                <div className={`${tab == 'Overall' ? 'block' : 'hidden'} mt-12 text-gray-400`}>
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <h1>Last air date: {obj.last_air_date}</h1>
                                    </div >
                                </div>
        
                                <div className={`${tab == 'Cast' ? 'flex': 'hidden'} mt-12 w-fit mx-auto flex-wrap gap-8`}>
                                    {cast.map ((e, id) => (
                                ...
                                <div className={`${tab == 'Seasons' ? 'block' : 'hidden'} mt-8 h-56 overflow-y-scroll`}>
                                    {obj.seasons.map ((e, id) => (
                                        <div key={id} className='my-10 text-white flex justify-between'>
                                            <div className="flex gap-4">
                                                <img className="h-40 rounded-lg" src={`https://image.tmdb.org/t/p/original${e?.poster_path}`} alt="" />
                                                <div className="my-auto">
                                                    <h1 className="my-6">{e.name}</h1>
                                                    <h1 className="my-6 text-gray-400">{e.air_date}</h1>
                                                </div>
                                            </div>
                                ...
                                <div className="lg:hidden mt-12 text-white">
                                    ...
                                    <div className="mt-6 max-h-[1000px] overflow-y-scroll">
                                        {similar.map ((e, id) => (
                                            <Link to={`/tv/detail/${e.id}`} key={id} className='flex gap-6 hover:brightness-75 my-6 duration-300'>
                                                <img className="h-40 rounded-lg" src={`https://image.tmdb.org/t/p/original${e.poster_path}`} alt="" />
                                                <div className="my-auto">
                                                    <h1>{e.name}</h1>
...
export default memo(DetailTVShow)