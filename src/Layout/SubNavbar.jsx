...
function SubNavbar ({page}) {
    const [taggleNav, setTaggleNav] = useState(false)
    ...
    useEffect (() => {
        if (taggleNav) {
            objRef.contain.current.className = objRef.contain.current.className.replace('translate-x-[-260px]', 'translate-x-0');
            objRef.overlay.current.className = objRef.overlay.current.className.replace('hidden', 'none')
        } else {
            objRef.contain.current.className = objRef.contain.current.className.replace('translate-x-0', 'translate-x-[-260px]');
            objRef.overlay.current.className = objRef.overlay.current.className.replace('none', 'hidden')
        }
    }, [taggleNav])

    const handleTaggleNav = () => {
        setTaggleNav(prev => !prev)
    }
    ...
    const handleRedirectBookmarked = (e) => {
        e.preventDefault ()
        if (!user.email) {
            objRef.noti.current.className = objRef.noti.current.className.replace ('hidden', 'none')
            setTimeout (() => {
                objRef.noti.current.className = objRef.noti.current.className.replace ('none', 'hidden')
            }, 1000)
        } else {
            navigate ('/bookmarked')
        }
    ...
    return  (
        <>
        <div ref={objRef.noti} className="hidden">
            <ErrorNoti />
        </div>
        
        <div onClick={handleTaggleNav} className="md:hidden absolute z-30 right-6 top-6 cursor-pointer">
            <div className="h-1 w-6 my-1 border-2 rounded-sm bg-gray-400" />
            <div className="h-1 w-6 my-1 border-2 rounded-sm bg-gray-400" />
            <div className="h-1 w-6 my-1 border-2 rounded-sm bg-gray-400" />
        </div>
        <div ref={objRef.overlay} onClick={handleTaggleNav} className="absolute inset-0 bg-black z-20 opacity-60 hidden"></div>
        <div ref={objRef.contain} className="md:hidden fixed z-30 translate-x-[-260px] duration-300 max-w-[260px] w-[70vw] h-screen pl-8 pt-6 bg-[#333335]">
